       ...
       case ('QU2P')
          if (size(map1,2) == 3) then
             do i = 0, size(map1,1)-1
                if (all(map1(i,2:3) /= missval)) then
...
!!$          end if
       case ('amp2mask')
          where (map1 < value .or. map1 == missval) 
             map1 = 0.d0
          elsewhere
             map1 = 1.d0
          end where
       ...
       case ('hitcount2mask')
          write(*,*) 'Number of exposed pixels  = ', count(map1 /= missval)
          where (map1 <= maxval(map1) / value**2 .or. map1 == missval) 
             map1 = 0.d0
          elsewhere
             map1 = 1.d0
          end where
