module quiet_Lx_mod
  ...
  type Lx_struct
     ...
     integer(i4b)                                 :: nside, coord_sys
     real(dp)                                     :: scanfreq
     real(sp),     allocatable, dimension(:,:,:)  :: point  ! Gal:(phi/theta/psi,time,mod)
     integer(i4b), allocatable, dimension(:)      :: pixels ! (nhit)
     real(sp),     allocatable, dimension(:,:,:)  :: point_objrel ! (phi/theta/psi,time,obj)

     real(dp),     allocatable, dimension(:)      :: time_gain            ! (time)
     real(sp),     allocatable, dimension(:,:)    :: gain                 ! (time, diode)
     real(dp),     allocatable, dimension(:)      :: sigma0, alpha, fknee ! (diode)
     real(dp),     allocatable, dimension(:,:,:)  :: corr                 ! (bin,diode,diode)
     real(dp),     allocatable, dimension(:)      :: corr_freqs           ! (bin)
     real(dp)                                     :: stats(STAT_NUM)
     real(dp),     allocatable, dimension(:,:)    :: diode_stats          ! (diode,stat)
     real(dp),     allocatable, dimension(:,:)    :: filter_par           ! (diode,param)
