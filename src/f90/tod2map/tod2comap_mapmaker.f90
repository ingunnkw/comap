       ...
       do j = 1, ndet
          ...
          do sb = 1, nsb
             do freq = 1, nfreq
                ...
                do sim = 1, tod%nsim
                   ! Simulattions 
                   if (tod%rms_sim(freq,sb,j,sim) == 0.d0) cycle
                   map_scan%dsum_sim(p,q,freq,sb,det,sim) = map_scan%dsum_sim(p,q,freq,sb,det,sim) + 1.d0 / tod%rms_sim(freq,sb,j,sim)**2 * tod%d_sim(i,freq,sb,j,sim) 
                   map_scan%div_sim(p,q,freq,sb,det,sim) = map_scan%div_sim(p,q,freq,sb,det,sim) + 1.d0 / tod%rms_sim(freq,sb,j,sim)**2
                end do
                !end if
             end do
          end do
